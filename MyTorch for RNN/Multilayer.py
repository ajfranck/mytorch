...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.rnn = nn.LSTM(input_size, hidden_size, num_layers=2)  # Set num_layers to 2
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = (
            torch.zeros(2, batch_size, self.hidden_size).to(device),  # Set the first dimension to 2
            torch.zeros(2, batch_size, self.hidden_size).to(device)   # Set the first dimension to 2
        )
        return hidden
...
multilayer_perplexity = math.exp(average_loss)

print(f"Perplexity LTSM: {multilayer_perplexity}")
